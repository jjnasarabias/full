resource "azurerm_policy_definition" "namespace_to_pod_label_mutation" {
  for_each = local.namespaceLabelsMap

  name         = "${local.labelsDomain}_${each.value.label}_${each.value.value}-from-namespace-to-pod"
  policy_type  = "Custom"
  mode         = "Microsoft.Kubernetes.Data"
  display_name = "Add namespace ${each.value.label} label to pod"
  description  = "Add namespace ${local.labelsDomain}/${each.value.label}: ${each.value.value} label to workload pod template spec"
